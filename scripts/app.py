...
with st.sidebar:
    ...
    if user_input and user_input != st.session_state.input_guardado:
        ...
        animacion_thinking = cargar_animacion_local("./assetss/newload.json")  # asegúrate que exista ese archivo
        if animacion_thinking:
            st_lottie(animacion_thinking, height=120, key="typing")
            time.sleep(5.0)
    ...
    with st.expander("Ver detalles del modelo"):
        ...
        **Dataset:** Swedish Leaf Dataset (7 clases)  
        **Entrenamiento:** 5 épocas + fine-tuning  
        **Preprocesamiento:** Imágenes 224x224 px, label smoothing  
        **Framework:** TensorFlow + Keras  
        """)
        if st.button("📊 Ver matriz de confusión"):
            st.image("data/confusion_matrix.png", caption="Matriz de Confusión del Modelo")
